...
class Arms:
    ...
    @staticmethod
    def convert_from_state(ee_pose, joint_pose):
        return { "position" :
                { "x" : ee_pose.position.x,
                  "y" : ee_pose.position.y,
                  "z" : ee_pose.position.z
                }, "orientation": {
                  "x" : ee_pose.orientation.x,    
    ...
    @staticmethod
    def convert_to_state(dic, refFrame=ArmState.ROBOT_BASE):
        return ArmState(refFrame, Pose(Point(dic["position"]["x"],
    ...
    def solve_ik_for_arm(arm_index, arm_state, cur_arm_pose=None):
	    ...
	    #rospy.loginfo('solve_ik_for_arm: Arm ' + str(arm_index) + ' is relative')
            ...
            target_ee_pose = Pose(Point(
                        ...
                        Quaternion(
                            arm_state.ee_pose.orientation.x,
                            arm_state.ee_pose.orientation.y,
                            arm_state.ee_pose.orientation.z,
                            arm_state.ee_pose.orientation.w))
            
            target_joints = Arms.arms[arm_index].get_ik_for_ee(target_ee_pose,
                                            arm_state.joint_pose)
            
            if (target_joints == None):
                rospy.logerr('No IK for pose relative end-effector pose.')
                return solution, False
            else:
                solution.refFrame = ArmState.ROBOT_BASE
    ...
    def execute_rec_action(self, action):
        if (action.type == Action.ACTION_QUEUE):
            ...
            if (has_solution_l and has_solution_r):
                self.move_to_joints(r_arm, l_arm)
        elif (action.type == Action.TRAJECTORY):
            for a_ind in [0, 1]:
                Arms.arms[a_ind].exectute_joint_traj(
                    [Arms.solve_ik_for_arm(a_ind, 
                    Arms.convert_to_state(pose["arms"][a_ind], ArmState.PREVIOUS_POSE),
                    Arms.get_ee_state(a_ind))[0] for pose in action.poses],
                    [rospy.Duration(0, pose["timing"]) for pose in action.poses])
