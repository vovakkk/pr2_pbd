...
class Session:
    ...
    def _get_experiment_state(self):
        ...
        return ExperimentState(
            self.actions[self.current_action_index].id,
            self._selected_step)
...
#         return self.actions[self.current_action_index].get_name()
    ...
    def clear_current_action(self):
        ...
        if (self.n_actions() > 0):
            self.actions[self.current_action_index].actions.clear()
        else:
    ...
    def add_step_to_action(self, step, object_list):
        ...
        if (self.n_actions() > 0):
            self.actions[self.current_action_index].actions.push(step)
        else:
            rospy.logwarn('No skills created yet.')
        #self._object_list = object_list
        self._update_experiment_state()
    ...
    def append_action_step(self, action_step):
        self.actions[self.current_action_index].actions.insert(
            self._selected_step, action_step)