    ...
    def disp_action(self, act):
        ...
        for ind, sub_act in enumerate(act.actions):
            def createLayout(ind, sub_act):
                ...
                self.stepsGrid.addWidget(typeBox, ind, 0)
                
                def setRow(act_type):
                    for itm in [self.stepsGrid.itemAtPosition(ind, i)
                            for i in range(1, 4)]:
                        if itm:
                            wid = itm.widget()
                            self.stepsGrid.removeWidget(wid)
                    ...
                    if (act_type == Action.ACTION_QUEUE):
                        ...
                        self.stepsGrid.addWidget(act_name, ind, 1)
                        ...
                        self.stepsGrid.addWidget(rec_but, ind, 1)
                        ...
                        def hand_rec():
                            ...
                            #command = Command()
                            #command.command = Command.SAVE_POSE
                            #self.speech_cmd_publisher.publish(command)
                        rec_but.clicked.connect(hand_rec)
                        self.stepsGrid.addWidget(rec_but, ind, 1)
